...
export class ProviderWebhookService implements OnModuleDestroy {
  ...
  constructor(
    ...
    this.webhook = axios.create({
      baseURL: this.workerConf.providerBaseUrl.toString(),
    });

    this.webhook.defaults.headers.common.Authorization = this.workerConf.providerApiToken;
  }
  ...
  private async checkProviderWebhook() {
    ...
    if (this.failedHealthChecks > 0) {
      if (this.failedHealthChecks >= this.workerConf.healthCheckMaxRetries) {
        this.logger.error(
          `FATAL ERROR: Failed to connect to provider webhook at '${this.workerConf.providerBaseUrl}' after ${this.failedHealthChecks} attempts.`,
        );
      ...
      this.schedulerRegistry.addTimeout(
        ...
        setTimeout(
          ...
          Math.min(
            ...
            this.workerConf.healthCheckMaxRetryIntervalSeconds * MILLISECONDS_PER_SECOND,
          ),
        ),
      );
    } else if (this.successfulHealthChecks > 0) {
      if (this.successfulHealthChecks >= this.workerConf.healthCheckSuccessThreshold) {
        ...
        this.schedulerRegistry.addTimeout(
          HEALTH_CHECK_TIMEOUT_NAME,
          setTimeout(() => this.checkProviderWebhook(), this.workerConf.webhookRetryIntervalSeconds),
        );
      }
    }
  }
