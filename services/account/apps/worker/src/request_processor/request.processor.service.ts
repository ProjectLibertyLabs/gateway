...
@Injectable()
@Processor(QueueConstants.ACCOUNT_CHANGE_REQUEST_QUEUE)
export class RequestProcessorService extends BaseConsumer {
  constructor(
    @InjectRedis() private cacheManager: Redis,
    @InjectQueue(QueueConstants.ACCOUNT_CHANGE_PUBLISH_QUEUE) private accountChangePublisherQueue: Queue,
  ...
  async process(job: Job<ProviderGraphUpdateJob, any, string>): Promise<void> {
    ...
    try {
      ...
      this.accountStateManager.removeUserGraph(dsnpUserId.toString());
      await this.accountStateManager.importBundles(dsnpUserId, job.data.graphKeyPairs ?? []);
      ...
      try {
        ...
        this.accountStateManager.applyActions(actions, true);
      } catch (e: any) {
      ...
      const exportedUpdates = this.accountStateManager.exportUserGraphUpdates(dsnpUserId.toString());
      ...
      graphPublisherJobs.forEach((graphPublisherJob) => {
        this.accountChangePublisherQueue.add(`Graph Publisher Job - ${graphPublisherJob.referenceId}`, graphPublisherJob);
      });

      const reImported = await this.accountStateManager.importBundles(dsnpUserId, job.data.graphKeyPairs ?? []);
      if (reImported) {
        ...
        // eslint-disable-next-line no-await-in-loop
        const userGraphExists = this.accountStateManager.graphContainsUser(dsnpUserId.toString());
        if (!userGraphExists) {
  ...
  private async importConnectionKeys(graphConnections: ConnectionDto[]): Promise<void> {
    const keyPromises = graphConnections
      ...
      .map(({ dsnpId }) => this.accountStateManager.formDsnpKeys(dsnpId));
    ...
    this.accountStateManager.importUserData(bundles);
  }
  ...
  async formConnections(
    ...
    const dsnpKeys: DsnpKeys = await this.accountStateManager.formDsnpKeys(dsnpUserId);
    ...
    await Promise.all(
      graphConnections.map(async (connection): Promise<void> => {
        ...
        const schemaId = this.accountStateManager.getSchemaIdFromConfig(connectionType as ConnectionType, privacyType as PrivacyType);
